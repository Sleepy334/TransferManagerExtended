...
namespace TransferManagerCore {
    ...
    public class UIKeymappingsPanel : UICustomControl {
        internal UIComponent AddKeymapping(string label, SavedInputKey savedInputKey, OnValueChanged eventHandler) {
            ...
            m_eventValueChanged = eventHandler;
            return uibutton;
        }
        ...
        private void OnBindingKeyDown(UIComponent comp, UIKeyEventParameter p) {
            if (this.m_EditingBinding is not null && !IsModifierKey(p.keycode)) {
                p.Use();
                UIView.PopModal();
                ...
                if (m_eventValueChanged != null)
                {
                    m_eventValueChanged(0);
                }
        ...
        private void OnBindingMouseDown(UIComponent comp, UIMouseEventParameter p) {
            if (this.m_EditingBinding is null) {
                p.Use();
        ...
        private int count;
        private OnValueChanged? m_eventValueChanged;
    }
}
